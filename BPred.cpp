...
PredType BPNotTaken::predict(const Instruction * inst, InstID oracleID, bool doUpdate)
{
    ...
    if (index == -1) {
        branchMiss[instCount] = 0;
        branchHit[instCount] = 0;
        InstructionID[instCount] = iID;  
        index = instCount;
        instCount++;
    }
...
BPNotTaken::~BPNotTaken()
{
    int branchIns[4];
    int branchMisses[4];
    int branchHitsCorrect[4];

    for(int i = 0; i < 4; i++) {
        branchIns[i] = 0;
        branchMisses[i] = 0;
        branchHitsCorrect[i] = 0;
    }

    for(int i = 0; i < instCount; i++) {
        int instructionRunCount = branchMiss[i] + branchHit[i];

        if (instructionRunCount >= 2000) {
            branchIns[3]++;
            branchMisses[3] += branchMiss[i];
            branchHitsCorrect[3] += branchHit[i];
        } else if (instructionRunCount >= 200) {
            branchIns[2]++;
            branchMisses[2] += branchMiss[i];
            branchHitsCorrect[2] += branchHit[i];
        } else if (instructionRunCount >= 20) {
            branchIns[1]++;
            branchMisses[1] += branchMiss[i];
            branchHitsCorrect[1] += branchHit[i];
        } else {
            branchIns[0]++;
            branchMisses[0] += branchMiss[i];
            branchHitsCorrect[0] += branchHit[i];
        }
    }

    std::cout << "Not Taken Predictor" << std::endl;
    std::cout << "1-19 Total: " << branchMisses[0] + branchHitsCorrect[0] << " Correct: " << branchHitsCorrect[0] << " Incorrect:" << branchMisses[0] << " Instructions: " << branchIns[0] << "   Accuracy: " << 1.0 * branchHitsCorrect[0] / (branchMisses[0] + branchHitsCorrect[0]) << std::endl;
    std::cout << "20-199 Total: " << branchMisses[1] + branchHitsCorrect[1] << " Correct: " << branchHitsCorrect[1] << " Incorrect:" << branchMisses[1] << " Instructions: " << branchIns[1] << "   Accuracy: " << 1.0 * branchHitsCorrect[1] / (branchMisses[1] + branchHitsCorrect[1]) << std::endl;
    std::cout << "200-1999 Total: " << branchMisses[2] + branchHitsCorrect[2] << " Correct: " << branchHitsCorrect[2] << " Incorrect:" << branchMisses[2] << " Instructions: " << branchIns[2] << "   Accuracy: " << 1.0 * branchHitsCorrect[2] / (branchMisses[2] + branchHitsCorrect[2]) << std::endl;
    std::cout << "2000+ Total: " << branchMisses[3] + branchHitsCorrect[3] << " Correct: " << branchHitsCorrect[3] << " Incorrect:" << branchMisses[3] << " Instructions: " << branchIns[3] << "   Accuracy: " << 1.0 * branchHitsCorrect[3] / (branchMisses[3] + branchHitsCorrect[3]) << std::endl;
    std::cout << "" << std::endl;
...

{
    ...
    for (int i = 0; i < 5000; i++) {
        InstructionIDInit[i] = false;  // No instructions initialized yet
    }
...
{
    int branchIns[4] = {0};
    int branchMisses[4] = {0};
    int branchHitsCorrect[4] = {0};
...
PredType BPHybrid::predict(const Instruction *inst, InstID oracleID, bool doUpdate)
{
    ...
    for (int i = 0; i < instCount; i++) {
        if (InstructionID[i] == iID && InstructionIDInit[i]) {
            index = i;
            break;
        }
    }
    ...
    if (index == -1) {
        ...
        branchHit[instCount] = 0;
        InstructionID[instCount] = iID; 
        InstructionIDInit[instCount] = true;  
        instCount++;
    }
    ...
    bool globalTaken = doUpdate ? globalTable.predict(l2Index, taken) : globalTable.predict(l2Index);
    bool localTaken = doUpdate ? localTable.predict(iID, taken) : localTable.predict(iID);

    bool metaOut;
    if (!doUpdate) {
        metaOut = metaTable.predict(l2Index); 
