...
SMPCache::SMPCache(SMemorySystem *dms, const char *section, const char *name)
    ...
    , compMiss("%s:compMiss", name) //WOLF CODE ADDING compMiss
    , capMiss("%s:capMiss", name) //WOLF CODE ADDING capMiss
    , confMiss("%s:confMiss", name) //WOLF CODE ADDING confMiss
    ...
    , allocDirty("%s:allocDirty", name)
{
    ...
    if(SMPMemRequest::SMPMemReqStrMap.empty()) {
        ...
        SMPMemRequest::SMPMemReqStrMap[SharingWriteBack]=		   "SharingWriteBack"; //WOLF FIXME IS THIS SOMETHING TO ADD FOR OTHER VARIABLES?
        ...
        SMPMemRequest::SMPMemReqStrMap[WriteBackRequest]=	   "WriteBackRequest"; //WOLF FIXME IS THIS SOMETHING TO ADD FOR OTHER VARIABLES?
...
//WOLF CODE
void SMPCache::missClassifier(PAddr addr) {
    PAddr tag = calcTag(addr);
    bool compMiss_status = doCompMiss(addr);
    if (!compMiss_status) {
        bool capMiss_status = doCapMiss(addr);
        if (!capMiss_status) {
            doConfMiss(addr);
        }
    }
...
void SMPCache::updateTracker(PAddr addr) {
    PAddr tag = calcTag(addr);

    vector<PAddr>::iterator it;
    for (it = cap_cache_tracker.begin(); it != cap_cache_tracker.end(); ++it) {
        if (*it == tag) {
            cap_cache_tracker.erase(it);
            cap_cache_tracker.push_back(tag);
            return;
        }
    }
    //If maybe the tag isn't in our cache yet, we should add it in.
    cap_cache_tracker.push_back(tag);
    if(cap_cache_tracker.size() > cache->getNumLines()) {
        cap_cache_tracker.erase(cap_cache_tracker.begin());
    }
    return;
}
...
bool SMPCache::doCompMiss(PAddr addr)
{
    ...
    uint32_t addr_index = cache->calcIndex4Addr(addr);
...
bool SMPCache::doCapMiss(PAddr addr)
{
    PAddr tag = calcTag(addr);
    //uint32_t tag_set = cache->calcSet4Addr(addr);
    //uint32_t addr_index = cache->calcIndex4Addr(addr);
...
bool SMPCache::doConfMiss(PAddr addr)
{
    PAddr tag = calcTag(addr);
    confMiss.inc();
    return true;
}
//^^^ WOLF TODO VERIFY THESE FUNCTIONS. ^^^
