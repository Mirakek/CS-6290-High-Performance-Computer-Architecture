...
protected:
    ...
    //WOLF CODE
    // Counter for compulsory read miss in each core's DL1 cache
    GStatsCntr readCompMiss;
    // Counter for replacement (capacity or conflict) read miss in each core's DL1 cache
    GStatsCntr readReplMiss;
    // Counter for coherence read miss in each core's DL1 cache
    GStatsCntr readCoheMiss;
    // Counter for compulsory write miss in each core's DL1 cache
    GStatsCntr writeCompMiss;
    // Counter for replacement (capacity or conflict) write miss in each core's DL1 cache
    GStatsCntr writeReplMiss;
    // Counter for coherence write miss in each core's DL1 cache
    ...
    std::vector<PAddr> compulsive_miss_tracker; // Did we ever store the tag before, read edition?

    std::map<Line*, PAddr> invalidated_tag_per_line; // Is a line currently invalidated? What was the last element stored in the line?
    ...
    //WOLF CODE
    // Each of these has the address taken in, as well as determines whether we are dealing with a read or write miss (the type).
    void missClassifier(PAddr addr, bool missType, Line* l);
    void updateTracker(Line* l);
    void updateInvalidate(Line* l, PAddr tag);
    bool doCoheMiss(PAddr addr, bool missType, Line* l);
    bool doCompMiss(PAddr addr); // Determine if we have a compulsory miss.
    //WOLF CODE END
