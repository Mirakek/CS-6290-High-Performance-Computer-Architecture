...
void SMPCache::updateTracker(Line* l) {

    //Update tags for current invalid line list.
    if(invalidated_tag_per_line.find(l) != invalidated_tag_per_line.end()) {
        invalidated_tag_per_line.erase(l);
    }

    return;
}

void SMPCache::updateInvalidate(Line* l, PAddr tag) {


    //Update tags for current invalid line list.
    if(invalidated_tag_per_line.find(l) == invalidated_tag_per_line.end()) {
        invalidated_tag_per_line.insert(make_pair(l, tag));
    }
    else {
        invalidated_tag_per_line[l] = tag;
    }
    return;
}


//If we have any line that is currently invalid, used to be valid, and last contained what we are looking for, return true for coherence miss.
bool SMPCache::doCoheMiss(PAddr addr, bool missType, Line* l) {
    PAddr tag = calcTag(addr);

    // If there exists a line prior that is currently invalid and has the tag we seek:
    for (auto i : invalidated_tag_per_line) {
        if (i.second == tag) {
            return true;
        }
    }
...
// Function to see if we have a compulsive miss; if so, add to counter, compulsive tracker list, and return true.
bool SMPCache::doCompMiss(PAddr addr) {
    PAddr tag = calcTag(addr);
    uint32_t tag_set = cache->calcSet4Addr(addr);

    if(compulsive_miss_tracker.empty()) {
        compulsive_miss_tracker.push_back(tag);
        return true;
    }
    else {
        bool comp_exist = false;
        for (int i = 0; i < compulsive_miss_tracker.size(); i++) {
            if(compulsive_miss_tracker[i] == tag) {
                comp_exist = true;
                break;
            }
        }
...
//Function to deal with different misses. The function takes in the address and the type of miss (read - 0, write - 1)
void SMPCache::missClassifier(PAddr addr, bool missType, Line* l) {
    if (doCoheMiss(addr, missType, l)) {
        missType ? writeCoheMiss.inc() : readCoheMiss.inc();
    }
    else if (doCompMiss(addr))
    {
        missType ? writeCompMiss.inc() : readCompMiss.inc();
    }
    else {
        missType ? writeReplMiss.inc() : readReplMiss.inc();
    }
    return;
}
...
void SMPCache::concludeAccess(MemRequest *mreq)
{
    ...
    if(pendingInv.find(taddr)!=pendingInv.end()) {
		...
		l->invalidate();
        updateInvalidate(l, tag); //WOLF CODE
        pendingInv.erase(taddr);
    }
